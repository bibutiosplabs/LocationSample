...
class MainActivity : AppCompatActivity() {
    ...
    private fun requestCurrentLocation() {
        ...
        if (applicationContext.hasPermission(Manifest.permission.ACCESS_FINE_LOCATION)) {
            ...
            val status = GoogleApiAvailability.getInstance().isGooglePlayServicesAvailable(this@MainActivity)
            ...
            when {
                ...
                GoogleApiAvailability.getInstance().isUserResolvableError(status) -> {
                    GoogleApiAvailability.getInstance().getErrorDialog(this@MainActivity, status, 123).show()
                }
                else -> {
                    logOutputToScreen("This device is not supported with google services")
                }
            }
        }
    }
